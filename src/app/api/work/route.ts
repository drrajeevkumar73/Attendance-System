  ...
  try {
    ...
    const existingTask = await prisma.todayswork.findFirst({
      where: {
        userId: user.id,
        createdAt: {
          gte: moment(currentDate).startOf('hour').set('hour', timeRange.start).toDate(),
          lte: moment(currentDate).endOf('hour').set('hour', timeRange.end).toDate(),
        },
      },
    });
      ...
      for (let { start, end } of timeRanges.slice(0, 4)) {
        const slotData = await prisma.todayswork.findFirst({
          where: {
            userId: user.id,
            createdAt: {
              gte: moment(currentDate).startOf('hour').set('hour', start).toDate(),
              lte: moment(currentDate).endOf('hour').set('hour', end).toDate(),
            },
          },
        });
