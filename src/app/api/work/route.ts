...
export async function POST(req: NextRequest) {
  try {
    ...
    // Set India time zone (Asia/Kolkata)
    const timeZone = 'Asia/Kolkata';
    
    // Get current date in India time
    const currentDate = new Date();
    const zonedDate = toZonedTime(currentDate, timeZone); // Convert to Asia/Kolkata timezone
    
    // Use this zonedDate for storing in the database
    ...
    // Save the task for the current slot, using zonedDate for India time
    const savedTask = await prisma.todayswork.create({
      data: {
        userId: user.id,
        content: data.content,
        createdAt: zonedDate, // Save in India time (Asia/Kolkata)
      },
    });
    ...
    if (currentHour >= 16 && currentHour < 18) {
      ...
      await prisma.attendance.create({
        data: {
          userId: user.id,
          createdAt: zonedDate, // Save attendance in India time
          status: isPresent ? "present" : "absent",
        },
      });
    }
