...
export async function POST(req: NextRequest) {
  try {
    ...
    const detaBYdipartment = await Promise.all(
      finDipartment.map(async (user) => {
        // Check the last task created or updated for the user
        const lastTask = await prisma.staffWork.findFirst({
        ...
        if (lastTask) {
          ...
          if (timeDiffInHours < 24) {
            return prisma.staffWork.update({
        ...
        return prisma.staffWork.create({
          data: {
            userId: user.id,
            task1,
            task2,
    ...
    return NextResponse.json(
      {
        success: true,
        message: "Tasks successfully created or updated for staff.",
        data: detaBYdipartment,
      },
      { status: 200 }
    );
  } catch (error: unknown) {
