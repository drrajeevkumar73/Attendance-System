  ...
  try {
    ...
    if (!currentTime.isBetween(startAllowedTime, endAllowedTime)) {
      return NextResponse.json(
        {
          success: false,
          message: "You can only add data between 8:00 PM and 8:00 AM.",
        },
        { status: 403 }
      );
    }
    ...
    const yesterdayStart = moment.tz("Asia/Kolkata").subtract(1, "days").startOf("day");
    ...
    const todayEntry = await prisma.telecaller.findFirst({
      where: {
        ...
        createdAt: {
          ...
          lte: todayStart.clone().endOf("day").toDate(),
        },
      },
    });

    // Ensure today's entry exists before allowing previous day
    if (!todayEntry && moment(currentDate).isSame(yesterdayStart, "day")) {
      return NextResponse.json(
        {
          success: false,
    ...
    // Check if currentDate is valid (today or yesterday)
    if (
