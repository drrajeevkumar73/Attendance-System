      ...
      {selectedTask === "work" ? (
        ...
        <TableHeader>
          <TableRow className="bg-primary">
            ...
            <TableHead className="border font-extrabold">10 AM - 1 PM</TableHead>
            <TableHead className="border font-extrabold">1 PM - 4 PM</TableHead>
            <TableHead className="border font-extrabold">4 PM - 7 PM</TableHead>
          </TableRow>
        </TableHeader>
        ...
        {loading ? (
          ...
          <TableBody>
            {data.map((day, dayIdx) => (
              <TableRow key={dayIdx}>
                {/* Date Column */}
                <TableCell className="border font-bold text-nowrap">{day.date}</TableCell>
    
                {/* Time Range Columns */}
                {["10 AM - 1 PM", "1 PM - 4 PM", "4 PM - 7 PM"].map((timeRange, idx) => (
                  <TableCell key={idx} className="border">
                    {day.timeRanges?.[timeRange]?.length > 0 ? (
                      day.timeRanges[timeRange].map((entry: any, entryIdx: any) => (
                        <p key={entryIdx} className="whitespace-pre-line break-words">
                          {entry}
                        </p>
                      ))
                    ) : (
